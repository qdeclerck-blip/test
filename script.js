/* ===========================
   ...
   Dynamic content loading from static JSON + animations
   =========================== */
...
(function () {
    ...
    function renderContent(c) {
        ...
        var pHTML = '';
        (c.concept.paragraphs || []).forEach(function (p) { pHTML += '<p class="concept-description">' + esc(p) + '</p>'; });
        ...
        (c.stats || []).forEach(function (s) {
            statsHTML += '<div class="stat"><span class="stat-number" data-target="' + s.value + '">0</span>' +
                '<span class="stat-unit">' + esc(s.unit) + '</span><span class="stat-label">' + esc(s.label) + '</span></div>';
        });
        ...
        (c.services.items || []).forEach(function (s) {
            svcHTML += '<div class="service"><div class="service-icon">' + (serviceIcons[s.icon] || serviceIcons.star) + '</div>' +
                '<h3 class="service-title">' + esc(s.title) + '</h3><p class="service-desc">' + esc(s.description) + '</p></div>';
        });
        ...
        (c.gallery.items || []).forEach(function (g, i) {
            galHTML += '<div class="' + (g.wide ? 'gallery-item gallery-item-wide' : 'gallery-item') + '" style="background:' + galleryGradients[i % galleryGradients.length] + '">' +
                '<span class="gallery-caption">' + esc(g.caption) + '</span></div>';
        });
        setHTML('galleryGrid', galHTML);

        var qel = document.getElementById('quoteText');
        if (qel) qel.innerHTML = '&laquo; ' + esc(c.quote) + ' &raquo;';
    ...
    window.addEventListener('load', function () { setTimeout(function () { loader.classList.add('hidden'); }, 2200); });
    ...
    document.querySelectorAll('a[href^="#"]').forEach(function (anchor) {
        anchor.addEventListener('click', function (e) {
            var tid = this.getAttribute('href'); if (tid === '#') return;
    ...
    function setupRevealAnimations() {
        ['.section-label','.section-title','.concept-description','.concept-visual','.maison-intro','.service','.decouvrir-card','.gallery-item','.season','.contact-info','.contact-form-wrapper','.quote'].forEach(function (sel) {
        ...
        var obs = new IntersectionObserver(function (entries) {
            entries.forEach(function (entry) {
                if (entry.isIntersecting) {
                    var p = entry.target.parentElement;
                    var sibs = Array.from(p.children).filter(function (c) { return c.classList.contains('reveal'); });
                    setTimeout(function () { entry.target.classList.add('visible'); }, sibs.indexOf(entry.target) * 100);
    ...
    function animateCounters() {
        var obs = new IntersectionObserver(function (entries) {
            entries.forEach(function (entry) {
                if (entry.isIntersecting) {
                    var el = entry.target, target = parseInt(el.getAttribute('data-target'), 10), st = null;
                    function step(ts) {
                        if (!st) st = ts; var p = Math.min((ts - st) / 2000, 1);
                        el.textContent = Math.floor((1 - Math.pow(1 - p, 4)) * target).toLocaleString('fr-FR');
        ...
        document.querySelectorAll('.stat-number').forEach(function (c) { obs.observe(c); });
    }
    ...
    (function () {
        ...
        window.addEventListener('scroll', function () {
            var s = window.scrollY; if (s < document.querySelector('.hero').offsetHeight) bg.style.transform = 'translateY(' + (s * 0.3) + 'px)';
        }, { passive: true });
    })();

    // --- Hero mouse ---
    (function () {
        var hero = document.querySelector('.hero'), g = document.querySelector('.hero-gradient'); if (!hero || !g) return;
        hero.addEventListener('mousemove', function (e) {
            var r = hero.getBoundingClientRect(), x = ((e.clientX - r.left) / r.width) * 100, y = ((e.clientY - r.top) / r.height) * 100;
            g.style.background = 'radial-gradient(ellipse at ' + x + '% ' + y + '%, rgba(139,115,85,.35) 0%, transparent 50%),radial-gradient(ellipse at 70% 80%, rgba(42,61,44,.5) 0%, transparent 50%),linear-gradient(to bottom, rgba(0,0,0,.15) 0%, rgba(0,0,0,.4) 100%)';
        });
    })();
    ...
    (function () {
        ...
        form.addEventListener('submit', function (e) {
            e.preventDefault(); var btn = form.querySelector('.form-submit'), orig = btn.innerHTML;
            btn.innerHTML = '<span>Message envoyé !</span>'; btn.style.background = '#3a5232'; btn.disabled = true;
